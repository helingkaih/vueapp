  ...
  <el-drawer v-model="drawer"
    ...
    <div>
      <el-row style="justify-content: center; margin-bottom: 22px">
        <el-button :type="mode === 1 ? 'primary' : 'default'"
                   @click="changeMode(1)"
                   style="width: 30%; font-size: 18px">登录</el-button>
        <el-button :type="mode !== 1 ? 'primary' : 'default'"
                   @click="changeMode(0)"
                   style="width: 30%; font-size: 18px">注册</el-button>
      </el-row>
    </div>
    <el-form ref="formRef"
             :model="form"
      ...
      <el-form-item label="密码验证"
                    v-if="mode !== 1">
        <el-input v-model="form.passwordCheck"
                  type="password"
                  placeholder="再次输入密码"
                  show-password></el-input>
      </el-form-item>
...
export default {
  ...
  setup() {
    ...
    const onSubmit = () => {
      ...
      reqkoa(mode.value !== 1 ? 'addUser' : 'checkUser', obj, 'POST').then(
        (data) => {
