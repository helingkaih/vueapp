<template>
  <div class="main-normal-box">
            ...
            <el-col :span="6">
              <el-form-item label="类型">
                <el-select v-model="form.type"
                  ...
                  <el-option v-for="(value, key) of types"
                             :key="key"
                             :label="value"
                             :value="key">
                  </el-option>
            ...
            <el-col :span="6">
              <el-form-item label="状态">
                <el-select v-model="form.state"
                  ...
                  <el-option v-for="(value, key) of states"
                             v-show="key !== '3'"
                             :key="key"
                             :label="value"
                             :value="key">
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <p style="
            ...
            <el-date-picker v-model="viewDay"
            ...
            <el-table-column label="操作"
              ...
              <template #default="scope">
                <el-button v-if="scope.row.state !== '1'"
                           ...
                           size="small"
                           plain
                           @click="handleClick('1', scope.row.id)">开始</el-button>
                <el-button v-else
                           type="primary"
                           size="small"
                           plain
                           @click="handleClick('2', scope.row.id)">暂停</el-button>
                <el-button type="success"
                           plain
                           size="small"
                           @click="handleClick('3', scope.row.id)">完成</el-button>
            ...
            <el-table-column label="详情"
              ...
              <template #default="scope">
                <el-dropdown>
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item>总耗时：{{ scope.row.takeUpTime }} 分</el-dropdown-item>
                      <el-dropdown-item>
                        类型：{{ types[scope.row.type] }}
                      </el-dropdown-item>
                      <el-dropdown-item>创建时间：{{ scope.row.createTime }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
              </template>
    ...
    <div v-else
      ...
      <div>
        <el-image key="CycleProcess"
                  :src="require('@/assets/bqb/jdxq.jpg')"
                  :preview-src-list="[require('@/assets/bqb/jdxq.jpg')]"></el-image>
        <p style="
            text-align: center;
            font-weight: bold;
            font-size: 24px;
...
export default {
  ...
  setup() {
    ...
    const submitWork = () => {
      ...
      reqkoa('addWork', workEditObj, 'POST').then((res) => {
        if (res.code === 1) {
          // 添加后还原表单数据
          form.name = ''
          form.state = '0'
     ...
     */
    const deleteWork = (id) => {
      reqkoa('deleteWork', { id }, 'POST').then((data) => {})
    }
    ...
    const changeWork = (item) => {
      reqkoa('changeWork', item, 'POST').then((data) => {})
    }
    ...
    const getWorkList = (date) => {
      ...
      reqkoa('getWork', obj, 'POST').then((res) => {
        finshData.value = []
        todoData.value = []
